...
autoPtr<electrolyteModel>
electrolyteModel::New
(
...
)
{
    ...
    {
        dict.lookup("typeName") >> modelName;
    }
    ...
    if (cstrIter == dictionaryConstructorTablePtr_->end())
    {
        FatalErrorIn
        (
            "electrolyteModel::New(const dictionary&, "
            "concReactionThermo&, const incompressible::turbulenceModel&)"
        )   << "Unknown electrolyteModel type "
            << modelName << endl << endl
            << "Valid  electrolyteModels are : " << endl
            << dictionaryConstructorTablePtr_->toc()
            << exit(FatalError);
  }

  return autoPtr<electrolyteModel>
