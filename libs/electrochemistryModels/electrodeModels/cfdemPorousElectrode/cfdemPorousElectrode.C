...
#include "cfdemPorousElectrode.H"
...
defineTypeNameAndDebug(cfdemPorousElectrode, 0);
...
cfdemPorousElectrode::cfdemPorousElectrode
(
...
void cfdemPorousElectrode::correctTransferCurrentDensity()
{
    ...
    {
        FatalErrorIn("cfdemPorousElectrode::calculateTransferCurrentDensity()")
    ...
    else
    {
        if (phiEl.dimensions() == dimMass*dimArea/pow3(dimTime)/dimCurrent)
        {
            ...
            forAll(iTrans_, cellI)
            {
                if(surfaceDensity_[cellI] > SMALL)
                {
                    ...
                    scalar eta = phiEs_[cellI] - phiEl[cellI] - eqPotential_
                        + R*T[cellI]/(ne*F)*log(redFactor/oxFactor);
                    //if (abs(potentialDiff) < abs(eqPotential_))
                    //{
                    i0 *=pow(oxFactor,1.0-alphaA_)*pow(redFactor,alphaA_);
                    iTrans_[cellI] = surfaceDensity_[cellI]*i0
                       *(
                            exp(alphaA_*ne*F/R/T[cellI]*eta)
                            -exp(-(1.0-alphaA_)*ne*F/R/T[cellI]*eta)
                        );
        ...
        {
            FatalErrorIn(
                    "cfdemPorousElectrode::calculateTransferCurrentDensity()")
                << "dimensions of field "
                << phiEl.name() 
                << "are not correct"
                << exit(FatalError);
...
void cfdemPorousElectrode::correctPercolation()
{
    if(intervalCounter_ == percolationInterval_-1)
    {
        ...
        {
            newContact = false;
            for(unsigned int index=0; index<particleContacts.size(); ++index)
            {
...
void cfdemPorousElectrode::correctElectrodeSurface()
{
...
//bool cfdemPorousElectrode::read()
//{
//    return true;
//}
