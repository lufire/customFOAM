...
Foam::liquidElectrolyte<ThermoType>::liquidElectrolyte
(
    concReactionThermo& thermo,
    const incompressible::turbulenceModel& turbulence
)
:
    liquidMultiSpeciesTransportModel(thermo, turbulence),
...
{   
    ...
    //const word condDictName(modelType+"Coeffs");
    dictionary condDict(this->subDict("conductivityModel"));
    const word modelType(condDict.lookup("typeName"));

    Info << "Ionic conductivity model " << modelType << endl;
    ...
    //kappaModel_.set(new conductivityModel(*this, thermo.T(),thermo_, z_));
    kappa_.set(kappaModel_.kappa());
}
...
template<class ThermoType>
void Foam::liquidElectrolyte<ThermoType>::updateCoefficients()
{     
    DijModel_().update();
    kappaModel_->update();
    ...
    forAll(species(), i)
    {
        if (i != inertIndex_)
        {
            ...
            D_[i] = (1-x_[i]) / (tmpGamma + dimensionedScalar("SMALL", dimensionSet(0,-2,1,0,0), Foam::SMALL));

            const volScalarField& yi = thermo_.composition().Y(i);
...
template<class ThermoType>
Foam::scalar Foam::liquidElectrolyte<ThermoType>::correct
(
...
template<class ThermoType>
bool Foam::liquidElectrolyte<ThermoType>::read()
{
    if (regIOobject::read())
    {
        return true;
