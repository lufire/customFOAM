...
void Foam::liquidMaxwellStefan<ThermoType>::updateCoefficients()
{     
    DijModel_().update();
...
template<class ThermoType>
Foam::liquidMaxwellStefan<ThermoType>::liquidMaxwellStefan
(
    // A.Alexiou 2014
...
template<class ThermoType>
Foam::scalar Foam::liquidMaxwellStefan<ThermoType>::correct
(
...
template<class ThermoType>
bool Foam::liquidMaxwellStefan<ThermoType>::read()
{
    if (regIOobject::read())
    {
        return true;
