...
const ThermoType& Foam::concMultiComponentMixture<ThermoType>::constructSpeciesData
(
...
void Foam::concMultiComponentMixture<ThermoType>::calculateMassFractions()
{
    forAll(Y_, speciesI)
    {
        ...
        {
            Y_[speciesI].internalField()[cellI] = 
    ...
    forAll(Y_, speciesI)
    {
        ...
        forAll(Y_[speciesI].boundaryField(), patchI)
        {
            ...
            {
                Y_[speciesI].boundaryField()[patchI][faceI] = 
...
template<class ThermoType>
const ThermoType& Foam::concMultiComponentMixture<ThermoType>::cellMixture
(
    const label celli
) const
{
...
template<class ThermoType>
const ThermoType& Foam::concMultiComponentMixture<ThermoType>::patchFaceMixture
(
    const label patchi,
    const label facei
) const
...
template<class ThermoType>
const ThermoType& Foam::concMultiComponentMixture<ThermoType>::cellVolMixture
(
    const scalar p,
    const scalar T,
    const label celli
...
template<class ThermoType>
const ThermoType& Foam::concMultiComponentMixture<ThermoType>::
patchFaceVolMixture
(
    const scalar p,
    const scalar T,
...
template<class ThermoType>
void Foam::concMultiComponentMixture<ThermoType>::read
(
    const dictionary& thermoDict
)
{
