...
#include "MeekerSethSlipVelocityFvPatchVectorField.H"
...
Foam::MeekerSethSlipVelocityFvPatchVectorField::
MeekerSethSlipVelocityFvPatchVectorField
(
    const fvPatch& p,
...
MeekerSethSlipVelocityFvPatchVectorField
(
    const MeekerSethSlipVelocityFvPatchVectorField& pvf,
    const fvPatch& p,
    const DimensionedField<vector, volMesh>& iF,
...
void Foam::MeekerSethSlipVelocityFvPatchVectorField::updateCoeffs()
{
    if (updated())
    {
        return;
    ...
    const scalarField magTauW = 
        mag((tUprevp - tUprevpI)*patch().deltaCoeffs()*rho_*nup);

    const scalarField magTUpI = mag(tUpI);
    const vectorField direction = tUpI/max(magTUpI, VSMALL);
    const scalar rAlpha = 1.0/alpha_;
    scalarField velocity = magTauW*0.0;
    forAll(velocity, faceI)
    {
        ...
        else if(magTauW[faceI] > sg_ && magTauW[faceI] < tau0_)
        {
            velocity[faceI] = 
                Ustar_ * pow((magTauW[faceI] - sg_)/(tau0_ - sg_), rAlpha);
        }
        else
        {
            velocity[faceI] = 
                (magTauW[faceI] - tau0_) * beta_ + Ustar_;
        }
    }
    const vectorField Up = 
        min(velocity * exp(-gamma_ * lambdap), mag(tUprevpI)) * direction;
...
void Foam::MeekerSethSlipVelocityFvPatchVectorField::write(Ostream& os) const
{
...
//void Foam::MeekerSethSlipVelocityFvPatchVectorField::operator=
//(
