function [varargout] = readdump_wallgranlocal(varargin)
...
while feof(dump) == 0
	...
	if (strcmpi(id,'ITEM: TIMESTEP'))
		...
		if (strcmpi(id,'ITEM: NUMBER OF ENTRIES'))
			Nwallcontacts(i) = str2num(fgetl(dump));
		else
			if (strcmpi(id(1:16),'ITEM: BOX BOUNDS'))
				...
				if (strcmpi(id(1:13),'ITEM: ENTRIES'))
					for j = 1 : 1: Nwallcontacts(i)
						wallcontact_data(j,:,i) = str2num(fgetl(dump));
					end
					i=i+1;
				end
			end 
...
varargout{1}.Nwallcontacts = Nwallcontacts;
varargout{1}.x_bound = x_bound;
varargout{1}.y_bound = y_bound;
varargout{1}.z_bound = z_bound;
varargout{1}.wallcontact_data = wallcontact_data;
%------------------------------
fclose(dump);
